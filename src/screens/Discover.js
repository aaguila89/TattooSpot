...
const artists = [
  ...
  },
  {
    ...
    name: 'Luna Park',
    location: 'Queens, NY',
    styles: ['Watercolor', 'Abstract', 'Illustrative'],
    rate: '$160/hr',
    rating: '4.9',
    reviews: 112,
    match: 79,
...
function Discover({ setScreen }) {
  ...
  const filtered = artists.filter(artist => {
    const matchesFilter = activeFilter === 'All Styles' ||
      artist.styles.some(s => s.toLowerCase().includes(activeFilter.toLowerCase()));
    const matchesSearch = artist.name.toLowerCase().includes(search.toLowerCase()) ||
      artist.styles.some(s => s.toLowerCase().includes(search.toLowerCase()));
    ...
    <div className="page">
      ...
      <div className="content">
        <div className="section-header">
          ...
          <p className="page-sub">Matched to your style</p>
        </div>
        ...
        {filtered.map(artist => (
          <div className="artist-card" key={artist.id} onClick={() => setScreen('profile')}>
            <div className="artist-card-img" style={{ background: artist.bg }}>
              <div className="mock-tattoo">{artist.emoji}</div>
              <div className="style-badge">{artist.styles[0]}</div>
              {artist.available && <div className="avail-badge">Available</div>}
            </div>
            <div className="artist-card-body">
              ...
              <div className="artist-meta">
                ...
                <div className="artist-rating">⭐ {artist.rating} <span className="review-count">({artist.reviews})</span></div>
              </div>
